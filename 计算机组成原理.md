...
![image-20250509181244756](http://typora--001.oss-cn-nanjing.aliyuncs.com/img/typora/20250509181245010.png#400h)

**由于尾数M的数符决定了整个浮点数的正负，所以当整个浮点数为负，且浮点数绝对值最大时，得到的即为负数N最小值，绝对值越大，负数越小，绝对值越小，负数越大，要注意绝对值由`整个阶码E×尾数M的数值位`构成**
$$
\begin{cases}
		负数N最小值：阶码最大值×尾数负数最小值=-(阶码最大值×尾数数值位)=-(浮点数最大绝对值)\\
		负数N最大值：阶码最小值×尾数负数最大值=-(阶码最小值×尾数数值位)=-(浮点数最小绝对值)\\
	\end{cases}
$$
