...
​	**上节课中，我们介绍了异常和中断的处理过程，我们知道整个中断处理过程是硬件和软件协同实现的。 因此从本节课开始，我们用三次课的时间从硬件角度介绍如何给CPU增加中断处理功能。本节课我们介绍针对数据通路的改造，我们仍然以之前多次课中都采用的基于CPU内部单总线结构的数据通路为例进行介绍。**

![image-20250818223741449](https://img.tynote.cn/img/typora/20250819172540788.png#800w)

​	**为了将重点放在中断处理相关部件并方便画图，我们将该图中的算术逻辑单元ALU、程序状态字寄存器PSW、寄存器组等相关部件隐去，如图所示。**

![image-20250818223900470](https://img.tynote.cn/img/typora/20250818223902598.png#800w)

​	**我们在CPU内部单总线上挂接一个异常指令地址寄存器EPC，该寄存器用于保存断点，根据我们之前学习过的有关异常与中断的相关基本概念可知，断点可能是异常指令地址的PC值，也可能是下一条指令的地址，将该寄存器挂接在CPU内部单总线上的原因是它需要与程序计数器PC进行数据交换，也就是保存断点和返回断点，与挂接在CPU内部单总线上的其他寄存器类似也需要给该寄存器配套提供两个控制信号$EPC_{out}和EPC_{in}$。**

![image-20250818225717821](https://img.tynote.cn/img/typora/20250818225719491.png#800w)

​	**除异常指令地址寄存器EPC外还需要添加一个中断使能位寄存器IE，其值为1时开中断，,值为0时关中断，通过控制信号STI使IE置1，通过控制信号CLI使IE清零。**

![image-20250818225903816](https://img.tynote.cn/img/typora/20250818225905738.png#800w)

​	**此外还需要添加中断控制逻辑，用于中断识别和中断优先级处理。在本例中假设CPU支持四个外部中断源$S_A、S_B、S_C、S_D$，则这四个中断源中的一个或多个产生中断时，中断控制逻辑一方面产生中断请求信号同时还输出优先级最高的中断源的编号，简称为中断号。**

![image-20250818230403329](https://img.tynote.cn/img/typora/20250818230405817.png#800w)

​	**中断请求信号作为与门的其中一个输入，与门的另一个输入来自中断使能位寄存器IE，将与门的输出连接到控制器的中断输入端，这样当IE的控制信号STI有效时，IE被置1，这相当于将与门打开，来自中断控制逻辑的中断请求直接通过该与门连接到控制器的中断输入端，这就是开中断操作。**

![image-20250818230631681](https://img.tynote.cn/img/typora/20250818230633959.png#800w)

​	**我们来举例说明，假设在完成开中断操作后中断源$S_A$和$S_D$都产生了中断相应的信号为有效，则中断控制逻辑一方面产生中断请求信号，一表示有效，同时还输出这两个中断源中优先级高的中断源的中断号，我们假设$S_A$的优先级高于$S_D$中断请求信号，通过与门到达控制器的中断输入端，如图所示，此时控制器应进行中断响应的一系列控制操作，例如关中断保存断点，进入相应的中断服务程序。**

![image-20250818231803163](https://img.tynote.cn/img/typora/20250818231804917.png#800w)

​	**相应的我们还需要引入一个多路选择器，该多路选择器被用来选择四路固定的地址，也就是四个中断源各自相应的中断服务程序的入口地址，该多路选择器的选择输出控制端连接到中断控制逻辑的中断号输出端，由中断号来决定应将哪个中断服务程序的入口地址通过多路选择器进行输出，对于本例是将中断源$S_A$的中断服务程序入口地址进行输出。需要说明的是，采用多路选择器是为了便于同学们理解，而在具体实现时中断服务程序入口地址并不是固定的，可以通过中断向量表访问存储器来获取，我们将在后续I/O系统那一章进行介绍，至此我们就完成了对数据通路的改造。接下来我们还需要对控制器进行相应的改造，后续两节课我们将分别针对硬布线控制器和微程序控制器进行改造。**

![image-20250818231836999](https://img.tynote.cn/img/typora/20250818231838797.png#800w)

#### 硬布线控制器的改造

​	**在上节课中，我们介绍了为了给CPU增加中断处理功能而针对数据通路的改造，在此基础上，本节课我们们介绍针对三级时序硬布线控制器和现代时序硬布线控制器的改造。**

![image-20250818233948585](https://img.tynote.cn/img/typora/20250818233950366.png#800w)

##### 三级时序硬布线控制器的改造

​	**如图所示，这是我们之前在本章有关时序发生器的那节课给同学们介绍过的三级时序发生器的构建过程，其中这是针对变长指令周期所构建出的三级时序状态转换图，也就是有限状态机FSM，那么接下来呢我们就在此基础上进行改造，进而实现为采用三级时序硬布线控制器的CPU增加中断处理功能。**

![image-20250818234033098](https://img.tynote.cn/img/typora/20250818234034831.png#800w)

​	**我们需要在该状态转换图的基础上增加包含两个节拍电位的中断响应周期，如图所示。在中断响应周期的第一个节拍电位，也就是状态转换图中的状态$S_9$执行的操作是将中断使能位寄存器IE清零，实现关中断，同时还需要将程序计数器PC的值经CPU内部单总线传送到异常指令地址寄存器EPC中进行保存，实现保存断点。**

![image-20250819102352468](https://img.tynote.cn/img/typora/20250819102354426.png#800w)

![image-20250819102804808](https://img.tynote.cn/img/typora/20250819102806547.png#800w)

​	**我们将上述操作带入基于CPU内部单总线的数据通路的这幅图中进行演示，以方便同学们理解，假设图中展示的是某条指令执行完成后检查中断请求，发现中断源$S_A$产生了中断，控制器在收到中断请求后会发出这些控制信号，我们将其标记在图中，如图所示，由于IE的控制信号CLI有效，因此IE被清零，IE的输出作为与门的一个输入，此时相当于关闭了与门，尽管中断控制逻辑产生的中断请求信号通过与门的另一个输入端进入与门，但与门的输出为零，也就是控制器的中断输入端输入零，这样就实现了关中断，与此同时由于控制信号$PC_{out}$有效，其所控制的三态门打开，这样PC的内容就通过该三态门到达CPU内部的单总线，经过在单总线中的传送，PC的内容就被传送到了EPC，由于控制信号$EPC_{in}$有效，因此PC的内容被写入EPC，这样就实现了保存断点。**

![image-20250819103608679](https://img.tynote.cn/img/typora/20250819103617838.png#800w)

![image-20250819104009195](https://img.tynote.cn/img/typora/20250819104011343.png#800w)

​	**再来看中断响应周期的第二个节拍电位，也就是状态转换图中的状态$S_{10}$，执行的操作是将当前中断服务程序入口地址经CPU内部单总线传送到程序计数器PC，我们基于之前的状态开始执行中断响应周期的第二个节拍电位中应执行的操作，这是控制器发出的相关控制信号，我们将其标记在图中，如图所示，由于控制信号$IntAdr_{out}$有效，因此多路选择器输出的中断源$S_A$的中断服务程序的入口地址通过该三代门到达CPU内部的单总线，经过在单总线中的传送中断源$S_A$的中断服务程序的入口地址就被传送到了程序计数器PC，由于控制信号$PC_{in}$有效，因此中断源$S_A$的中断服务程序的入口地址就被写入了PC，至此中断响应周期的第二个节拍电位中所执行的操作完毕。**

![image-20250819104148009](https://img.tynote.cn/img/typora/20250819104644075.png#800w)

![image-20250819104925205](https://img.tynote.cn/img/typora/20250819104927215.png#800w)

​	**接下来进入的是取指周期的第一个节拍电位，也就是开始执行中断服务程序，为什么这么说呢请不要忘记了，我们刚刚分析过此时程序计数器PC的内容是中断源$S_A$的中断服务程序的入口地址，请思考一下中断服务程序执行完毕后如何返回到被中断的程序呢？答案是应在中断服务程序的最后安排一条中断返回指令，在MIPS32指令集中中断返回指令为eret，我们将这条指令添加到上面的状态转换图中，添加在该分支的理由是该指令是无操作数指令，因此没有计算周期，取指周期结束后就要进入执行周期。**

![image-20250819105626661](https://img.tynote.cn/img/typora/20250819105628599.png#800w)

​	**该指令所要执行的操作是将异常指令地址寄存器EPC的内容通过CPU内部单总线传送到程序计数器PC，实现返回断点，同时将中断使能位寄存器IE置1，实现开中断，另外还需要控制中断控制逻辑清除当前中断请求信号，上述相关操作可在一个节拍电位内完成，我们将其安排在执行周期的最后一个节拍电位，也就是状态转换图中的状态$S_8$，从本例中的这一点也可以看出传统三级时序的缺点，那就是时序划分固定，对于耗时较少的简单指令也只能安排同样的执行周期。**

![image-20250819113741618](https://img.tynote.cn/img/typora/20250819113744036.png#800w)

​	**我们基于之前的状态开始执行中断返回指令的相关操作。**

![image-20250819160143566](https://img.tynote.cn/img/typora/20250819160145897.png#800w)

​	**这是控制器发出的相关控制信号，我们将其标记在图中，如图所示，由于控制信号$CIrInt$(Clear Interrupt Reaquest)有效，因此中断控制逻辑的相关信号被清除，如图所示，由于控制信号$STI$有效，因此IE被置1，由于IE的输出连接在与门的其中一个输入，这相当于将与门打开，来自中断控制逻辑的中断请求直接通过该与门连接到控制器的中断输入端，这样就实现了开中断操作。另外由于控制信号$EPC_{out}$有效，因此其所控制的三态门打开，这样异常指令地址寄存器EPC中的内容，也就是之前在中断响应周期保存的PC值，或者说断点，通过该三态门到达CPU内部单总线，经过单总线的传送到达程序计数器PC，由于控制信号$PC_{in}$有效，因此断点被写入PC，这样就实现了返回断点的操作。**

![image-20250819162058207](https://img.tynote.cn/img/typora/20250819162100949.png#800w)

![image-20250819164623279](https://img.tynote.cn/img/typora/20250819164626227.png#800w)

​	**至此相信同学们已经理解了对变长指令周期三级时序的有限状态机进行改造，使其支持中断处理功能，在此基础上还要写出状态转换表，进而构建时序发生器，最终构建出支持中断的三级时序硬布线控制器，这些工作我们在之前有关三级时序硬布线控制器的几次课中已经给同学们介绍过了，就不再赘述了。**

![image-20250819165534355](https://img.tynote.cn/img/typora/20250819165536919.png#800w)

##### 现代时序硬布线控制器的改造

​	**在我们完成了针对三级时区硬布线控制器的改造后，请同学们对现代时序硬布线控制器进行改造，如图所示这是我们之前在本章有关现代时序硬布线控制器的那节课中给同学们介绍过的状态转换图，请同学们给该状态转换图添加相应状态，以支持中断功能。为了简单起见，我们隐去了每个状态所需的控制信号，提示一下，现在时序硬布线控制器不再采用多级时序体制，对操作控制信号的定时直接采用基本时钟信号，而不再采用基于基本时钟信号产生的节拍脉冲信号、节拍电位信号以及状态周期信号，给指令集中的每一条指令的执行过程都按需分配相应数量的时钟周期，以提高程序执行效率。**

![image-20250819165708774](https://img.tynote.cn/img/typora/20250819165711801.png#800w)

![image-20250819165754646](https://img.tynote.cn/img/typora/20250819165757064.png#800w)

​	**这两个新添加的状态，它们各自对应一个时钟周期，用于中断响应，也就是关中断、保存断点，进入相应的中断服务程序，这个新添加的状态eret对应一个时钟周期，用于中断返回，也就是返回断点、开中断，可以发现中断返回指令这种耗时较短的简单指令只需要一个时钟周期，采用现代时序可按需分配一个时钟周期，而传统三级时序由于时序划分固定，不得不分配三个节拍电位，相当于分配三个时钟周期。**

![image-20250819170248115](https://img.tynote.cn/img/typora/20250819170250942.png#800w)

​	**至此我们就得到了改造后的现代时序硬布线控制器的状态转换图，在此基础上还要写出状态转换表，进而构建出支持中断功能的现代时序硬布线控制器，这些工作在之前有关现代时序硬布线控制器的课中已经介绍过，就不再赘述了。**

![image-20250819170429567](https://img.tynote.cn/img/typora/20250819170432454.png#800w)

#### 微程序控制器的改造

​	**上节课中我们介绍了为了给CPU增加中断处理功能而针对三级时序硬布线控制器的改造以及针对现代时序硬布线控制器的改造，在此基础上呢本节课我们介绍针对微程序控制器的改造。我们以计数器法的微指令格式为例进行介绍，可以看到我们在操作控制字段中添加了六个与中断相关的微命令位，用于对应与中断相关的六个微操作控制信号，相信同学们并不陌生因为我们在上节课中对它们都进行了使用，另外由于指令执行完毕后要进行中断请求的检查，因此不管采用下址字段法还是计数器法，都必须在顺序控制字段中新增一个微指令结束判别测试位$P_{end}$，用于指明当前微指令为最后一条微指令，并根据中断测试条件进行分支，这样我们就得到了支持中断功能的微指令格式，基于微指令格式重新设计微指令并新增三条微指令，其中两条用于中断响应操作，一条用于中断返回，如图所示，这是我们上节课给同学们介绍过的支持中断功能的变长指令周期的三级时序状态转换图，其中这是中断响应周期中的两个节拍电位各自应该执行的操作，那么对于微程序控制器就需要增加两条用于中断响应操作的微指令，这是执行周期的第三个节拍应该执行的操作，用于实现中断返回，那么对于微程序控制器就需要增加一条用于中断返回的微指令，有了微指令就可以为指令集中的每条指令重新编写相应的微程序，以实现指令功能，相应的还需要修改微程序控制器的地址转移逻辑。**

![image-20250819171229858](https://img.tynote.cn/img/typora/20250819171232497.png#800w)

![image-20250819171450273](https://img.tynote.cn/img/typora/20250819171453036.png#800)

![image-20250819171536906](https://img.tynote.cn/img/typora/20250819171539641.png#800w)

![image-20250819174132744](https://img.tynote.cn/img/typora/20250819174135359.png#800w)

​	**我们直接给出修改好的微程序控制器的原理框图，可以看到在地址转移逻辑中给多路选择器增加了一个中断响应微程序入口地址的输入端，以便能够读取存放在控制存储器中的中断响应微程序，它由两条微指令构成，用于实现中断响应操作，另外该图还展示出了控制存储器中还存放有中断返回指令ERET的微程序，它由一条微指令构成，用于实现中断返回。**

![image-20250819190502534](https://img.tynote.cn/img/typora/20250819190504967.png#800w)

​	**这是多路选择器选择下址字段给出的地址作为下一个微地址的逻辑，其含义是当前微程序的最后一条微指令结束并且没有中断请求信号，这是多路选择器选择中断响应微程序入口地址作为下一个微地址的逻辑，其含义是当前微程序的最后一条微指令结束，有中断请求，没有其他判别测试条件成立，为匹配之前已修改好的微指令格式，对多路选择器输入端的扩充以及对多路选择器选择输出控制逻辑的修改，地址转移逻辑中的条件判别测试组合逻辑也要进行相应的修改，进而最终实现支持中断功能的微程序控制器。**

![image-20250819190513790](https://img.tynote.cn/img/typora/20250819190623149.png#800w)

### 中断服务程序

​	**之前我们用三次课的时间给同学们介绍了如何为CPU增加中断处理功能的相关知识包括对数据通路的改造对控制器的改造这些都属于硬件修改和扩充我们知道中断机制是硬件和软件协同的机制有了硬件支持还需要为各种中断请求编写相应的中断服务程序。**

![image-20250819191243872](https://img.tynote.cn/img/typora/20250819191246076.png#800w)

​	**本节课我们介绍中断服务程序主要包含的四个步骤步骤。**

![image-20250819192058846](https://img.tynote.cn/img/typora/20250819192101337.png#800w)

**请同学们注意中断服务需要遵循二八法则，也就是中断服务及完成20%的必要工作，80%转交后台处理，这并不难理解，例如按键处理中断，如果在中断服务中处理大量工作，则在中断服务过程中再次出现按键中断，则不能及时响应使用户感受到卡顿。**

​	**需要说明的是，以上给出的是针对简单的单级中断其中断服务程序所包含的四个步骤，根据之前的学习我们知道，进入中断服务程序之前是中断响应周期，所执行的操作是关中断、保存断点、进入中断服务程序，而在中断服务程序的最后一个步骤，也就是中断返回中所执行的操作是返回断点、开中断。**

![image-20250819192417450](https://img.tynote.cn/img/typora/20250819192419773.png#800w)

​	**这是我们上节课就学习过的内容，这是中断响应周期的第一个节拍电位，所执行的关中段和保存断点的操作，这是中断响应周期的第二个节拍电位，所执行的进入当前中断服务程序的操作，这是中断返回指令的执行周期，所执行的返回断点和开中断的操作。**

![image-20250819193214230](https://img.tynote.cn/img/typora/20250819193217216.png#800w)

​	**那么对于多级嵌套中断，保护现场后应该开中断，以确保中断服务程序还可以被中断，而恢复现场前应该关中断，以确保恢复现场的完整性，我们将这两个步骤标记在上图中。需要说明的是对于单级中断同时出现多个中断请求时，按中断优先级顺序处理，对于嵌套中断遵循优先级抢占规则，也就是高优先级中断可以中断低优先级中断的中断服务程序。**

![image-20250819193553290](https://img.tynote.cn/img/typora/20250819193556182.png#800w)
